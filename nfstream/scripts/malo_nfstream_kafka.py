...
class KafkaPublisherV1(NFPlugin):
    def __init__(self, **kwargs):
        ...
        #self.nom_file_snapshots=f"snapshots_{random.randint(1, 1000)}.csv"
        #self.file_snapshots = open(self.nom_fich, 'w')
        LOGGER.info (f"nombre del fichero:{self.nom_file_snapshots}")
    ...
    def on_init(self, packet, flow):
        #sleep (random.randint (1,100)/1000.0) # evitamos que los on_init de los distintos meters arranquen a la vez
        '''
        if not self.primer_pkt_recibido:
            self.primer_pkt_recibido=True
            self.first_timestamp_nfstream_secs=time()
            self.FIRST_TIMESTAMP_PCAP_secs= packet.time/1000.0 # viene expresado en milisegundos
            conn_id = f"{flow.src_ip}, {flow.dst_ip}, {flow.src_port}, {flow.dst_port}, {flow.protocol}"
            LOGGER.info (f"XXXXX primer_pkt_recibido. conn_id:{conn_id}")
            LOGGER.info (f"first pcap:{self.FIRST_TIMESTAMP_PCAP_secs}. first nfsstream:{self.first_timestamp_nfstream_secs}")
        '''
            
        flow.udps.l_snapshots_pendientes=[]   
    ...
    def procesa_snapshots_pendientes (self,flow,producer,flush):

        
        len_snap_pendientes=len(flow.udps.l_snapshots_pendientes)

        if len_snap_pendientes >0:
            
            conn_id = f"{flow.src_ip}, {flow.dst_ip}, {flow.src_port}, {flow.dst_port}, {flow.protocol}"
            #LOGGER.info (f"entra flow:{conn_id} len(flow.udps.l_snapshots_pendientes):{len(flow.udps.l_snapshots_pendientes)}")
            #LOGGER.info (f"entra flow:{conn_id} len(flow.udps.l_snapshots_pendientes):{len(flow.udps.l_snapshots_pendientes)}")
            len_ini=len(flow.udps.l_snapshots_pendientes)
            
            i = 0
            while i < len(flow.udps.l_snapshots_pendientes):
                snapshot = flow.udps.l_snapshots_pendientes[i]
    
                pkt_time= snapshot["metadata"]["timestamp"]/1000.0
                # packet.time expresado en milisegundos. Todos los tiempos dentro de nfstream van en milisegundos
                pkt_time_adjusted_secs= (pkt_time - self.FIRST_TIMESTAMP_PCAP_secs) + self.first_timestamp_nfstream_secs
                now_secs=time()
                ...
                if diff_secs > GAP :
                    ...
                    LOGGER.info (f"pkt_time:{datetime.fromtimestamp(pkt_time)}, pkt_time_adjusted_secs:{datetime.fromtimestamp(pkt_time_adjusted_secs)}, now_secs:{datetime.fromtimestamp(now_secs)}")
                    break
                else:
                    flow.udps.l_snapshots_pendientes.pop(i) # Quito el elemento de la lista
                    conn_id = f"{flow.src_ip}, {flow.dst_ip}, {flow.src_port}, {flow.dst_port}, {flow.protocol}"
                    
                    producer.send(topic=self.topic, value=snapshot, headers=[("version", "v1".encode("utf-8")), ("features_names", FEATURE_NAMES.encode("utf-8"))],
                                  timestamp_ms=int(time() * 1000), key=conn_id)
                    # cabecera="t_envio_kafka(datetime)*t_envio_kafka(timestamp)*last_packet_timestamp*t_preparacion_snap_nfstream*t_ini_intervalo*t_fin_intervalo*ip_src*ip_dst*port_src*port_dst*protocol"
            ...
            len_fin=len(flow.udps.l_snapshots_pendientes)
            if len_fin-len_ini >0:
                LOGGER.info (f"sale. procesa_snapshots_pendientes de flow:{conn_id}. entra con:{len_ini}. sale sin enviar len(flow.udps.l_snapshots_pendientes):{len(flow.udps.l_snapshots_pendientes)}")
            #LOGGER.info (f"sale flow:{conn_id} len(flow.udps.l_snapshots_pendientes):{len(flow.udps.l_snapshots_pendientes)}")
        ...
        if random.randint(1, 100) == 1:
            producer.send(topic=self.topic, value=data, 
                      headers=[("version", "v1".encode("utf-8")), 
                               ("features_names", FEATURE_NAMES.encode("utf-8"))], 
    ...
    def on_update(self, packet, flow, producer, flush):
        ...
        if packet.time > flow.udps.end_interval:   
            ...
            #LOGGER.info (f"XXX sn creado:{len(flow.udps.l_snapshots_pendientes)}")
            self.procesa_snapshots_pendientes (flow,producer,flush)

            # Ajusta timestamp del nuevo snapshot
    ...
    def on_expire(self, flow, producer, flush):
        ...
        LOGGER.info (f"KKKK expire. len(flow.udps.l_snapshots_pendientes):{len(flow.udps.l_snapshots_pendientes)}")
        self.procesa_snapshots_pendientes (flow,producer,flush)
        LOGGER.info (f"KKKK expire-2. len(flow.udps.l_snapshots_pendientes):{len(flow.udps.l_snapshots_pendientes)}")
        return
