...
RATIO_FLUSH_CLEANUP=300
RATIO_FLUSH_KAFKA = 900 # 999999999 #10000 # en paquetes
...
BATCH_SIZE_KAFKA = int(65536/2) #65536 # 65536*8 #int(65536/2) #65536 #int(65536/4)
LINGER_MS = 10 # 10
    ...
    while remaining and scanned < 1000:  # idle scan budget (each 10ms we scan 1000 as maximum)
        try:
            ...
            if flow.is_idle(meter_tick, idle_timeout):  # idle, expire it.
                channel.put(flow.expire(udps, sync, n_dissections, statistics, splt, ffi, lib, dissector, producer, True))
                del cache[flow_key]
                del flow
                scanned += 1
            else:
...
def meter_cleanup(cache, channel, udps, sync, n_dissections, statistics, splt, ffi, lib, dissector, producer):
    ...
    for flow_key in list(cache.keys())[:-1]:
        counter += 1
        if counter % RATIO_FLUSH_CLEANUP == 0:
            flush = True
            #print ("flush en cleanup")
        flow = cache[flow_key]
        # Push it on channel.
        channel.put(flow.expire(udps, sync, n_dissections, statistics, splt, ffi, lib, dissector, producer, flush))
        flush= False # Alberto. hay que volver a poner a False el flag de flush
        del cache[flow_key]
        del flow
    # Solo hemos dejado un elemento en cache (el ultimo)
    flow_key = list(cache.keys())[0]
    flow = cache[flow_key]
    # Push it on channel.
    channel.put(flow.expire(udps, sync, n_dissections, statistics, splt, ffi, lib, dissector, producer, True))
    del cache[flow_key]
...
def meter_workflow(source, snaplen, decode_tunnels, bpf_filter, promisc, n_roots, root_idx, mode,
    ...
    """ Metering workflow """
    flush = False
    flush_rate = RATIO_FLUSH_KAFKA #en paquetes
    #producer = KafkaProducer(bootstrap_servers=kafka_url, key_serializer=str.encode, value_serializer=dumps, buffer_memory=67108864*100, batch_size=BATCH_SIZE_KAFKA,linger_ms=LINGER_MS,max_in_flight_requests_per_connection=50)
    producer = KafkaProducer(bootstrap_servers=kafka_url, key_serializer=str.encode, value_serializer=dumps, batch_size=BATCH_SIZE_KAFKA,linger_ms=LINGER_MS,compression_type='gzip')
    '''
    producer = KafkaProducer(
        bootstrap_servers=kafka_url,
        key_serializer=str.encode, value_serializer=dumps,
        buffer_memory=67108864,
        #batch_size=32768,  # Increase batch size (32 KB)
        #linger_ms=10,      # Increase linger time to 10 ms
        #compression_type='gzip',  # Enable compression
        acks='1',          # Acknowledge receipt of message only after leader replication
        max_in_flight_requests_per_connection=5  # Increase max in-flight requests
    )
    '''
    #producer = KafkaProducer(bootstrap_servers=kafka_url, key_serializer=str.encode, value_serializer=dumps)
        ...
        while remaining_packets:
            ...
            if ret > 0:  # Valid must be processed by meter
                ...
                if packet_time > meter_tick:
                    ...
                    '''
                    if packet_time < meter_tick:
                        if meter_tick-nf_packet.time > 1:
                            print (f"meter_workflow. cambio del tiempo de un paquete. pkt.time:{nf_packet.time} < meter_tick:{meter_tick}. diff:{meter_tick-nf_packet.time}")
                    '''
                    nf_packet.time = meter_tick  # Force time order
                    
                if ret == 1:  # Must be processed
                    processed_packets += 1
                    if processed_packets % flush_rate == 0:
                        flush = True
                        # quitar solo para debug
                        #print (f"flush en meter_workflow. processed packets:{processed_packets}")
                    go_scan = False
                    ...
                    if go_scan:
                        idles = meter_scan(meter_tick, cache, idle_timeout, channel, udps, sync, n_dissections,
                                           statistics, splt, ffi, lib, dissector, producer)
                        active_flows -= idles
                else:  # time ticker
                    if meter_tick - meter_scan_tick >= meter_scan_interval:
                        idles = meter_scan(meter_tick, cache, idle_timeout, channel, udps, sync, n_dissections,
    ...
    meter_cleanup(cache, channel, udps, sync, n_dissections, statistics, splt, ffi, lib, dissector, producer)
    # Clean dissector
    lib.dissector_cleanup(dissector)
    # Release engine library
    channel.put(None)
    print(f"Process ended. Porcessed packets:{processed_packets}")
