...
class NFlow(object):
    ...
    def update(self, packet, idle_timeout, active_timeout, ffi, lib, udps, sync, accounting_mode,
        ...
        if ret > 0:  # If update done it will be zero, idle and active are matched to 1 and 2.
            ...
            return self.expire(udps, sync, n_dissections, statistics, splt, ffi, lib, dissector, producer, flush)  # expire it.
        if sync:  # If running with Plugins
            ...
            for udp in udps:  # Then call each plugin on_update entrypoint.
                udp.on_update(pythonize_packet(packet, ffi, self), self, producer, flush)
            if self.expiration_id == -1: # One of the plugins set expiration to custom value (-1)
                return self.expire(udps, sync, n_dissections, statistics, splt, ffi, lib, dissector, producer, flush)  # Expire it.

    def expire(self, udps, sync, n_dissections, statistics, splt, ffi, lib, dissector, producer, flush):
        ...
        if sync:  # Running with NFPlugins
            for udp in udps:
                udp.on_expire(self, producer, flush)  # Call each Plugin on_expire entrypoint
