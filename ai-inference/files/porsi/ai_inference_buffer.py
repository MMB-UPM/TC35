...
class AIInference:
    ...
    def __parse_message(self, message: Any, features_names: List[str], version: str) -> Tuple[pd.DataFrame, list]:
        ...
        if version == "v1":
            ...
            metadata = {**message_metadata['connection_id'], 'timestamp': message_metadata['timestamp']/1000, 'timestamp_nfstream': message_metadata['timestamp_nfstream'], 'timestamp_inference': datetime.timestamp(datetime.now()), 'flow_bytes': message_metadata['flow_bytes'], 'flow_pkts': message_metadata['flow_pkts'],
            ...
            if messages:
                ...
                for message in messages:
                    ...
                    if len(features_buffer)>=BUFFER_SIZE:
                        ...
                        probs[:,1]=0.1
                        probs[:,2]=0.2"""
                        #LOGGER.info(f"Arr Mio: {arr_mio.shape}")
